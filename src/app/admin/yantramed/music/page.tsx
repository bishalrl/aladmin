...
export default function MusicPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      setItems(await apiGet("/api/admin/yantramed/music"));
    } catch (e) {
  ...
  async function onCreate(e: FormEvent) {
    ...
    if (dayNumber) form.set("dayNumber", dayNumber);
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Audio stored on server disk for YantraMed"
      />
      <Card className="mb-6">
        <form onSubmit={onCreate} className="grid gap-3 md:grid-cols-2">
          ...
          <Input
            label="Course day (1–30, optional)"
            type="number"
            min={1}
            max={30}
            value={dayNumber}
